...
### add extra data 
big_master_table <-  read.csv(PATHS$clinical, header =TRUE, na.strings=c("","NA"))
...
clinical_table <- big_master_table[,c('PT', 'VISIT_n', 'PACKNO','BMI','PHADRES', 'DUR_DIS','AGE_DIAG','B_FEV1F','GINA', 
                                      ...
                                      'B_RVTLCPNVF',  'B_R520PNVR', 'B_SCONDPNVF','B_SACINPNVF', 'B_F50PNVR', 'DFEV1P')]
...
var_for_table <- c('gender','age','smoking.status','PACKNO','BMI','PHADRES', 'DUR_DIS','AGE_DIAG','B_FEV1F','GINA', 'NUM_EX','LAMA','BIO',
                   ...
                   'B_TLCPNVF', 'B_FEV1PNVG','B_FEV1FPNVG', 'DFEV1P', 'FENRES', 'PCD','bhr',
...
rownames(Final_statistics)[rownames(Final_statistics) == "B_FEV1FPNVG (mean (SD))"] = "FEV1/FVC % of predicted normal value (mean (SD))"
rownames(Final_statistics)[rownames(Final_statistics) == "DFEV1P (mean (SD))"] = "Bronchodilator reversibility, % initial,  (mean (SD))"
Final_statistics$NO = sub(', ', '-', Final_statistics$NO)
Final_statistics$YES = sub(', ', '-', Final_statistics$YES)
  
write.table(Final_statistics, file = paste0(ROOT_DIR, "/Exacerbations/Baseline_statistics_exacerbations.csv"),
